...
export function ContactEditModal({ contact, salesPeople, isAdminOrProcessor, isAdmin, onSave, onCancel }: ContactEditModalProps) {
  ...
  const Icon = typeIcons[editForm.type as keyof typeof typeIcons];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const baseData = {
        ...
        cell_phone: toNullIfEmpty(editForm.cell_phone),
        company: toNullIfEmpty(editForm.company),
        branch: toNullIfEmpty(editForm.branch),
        address: toNullIfEmpty(editForm.address),
        birthday: toNullIfEmpty(editForm.birthday),
        drinks: editForm.drinks,
        notes: toNullIfEmpty(editForm.notes),
      ...
      const adminData = canEditAdminFields ? {
        ...
        processor_notes: toNullIfEmpty(editForm.processor_notes),
      ...
      console.log('Branch value:', { editForm: editForm.branch, contact: contact.branch, final: formattedData.branch });
      console.log('Address value:', { editForm: editForm.address, contact: contact.address, final: formattedData.address });
      ...
      // Handle assignment changes (only when admin fields are enabled)
      if (canEditAdminFields) {
        const currentAssignmentId = contact.assignments?.[0]?.salesperson_id || contact.assigned_to || '';
        const hasAssignmentChanged = editForm.newSalespersonId !== currentAssignmentId;
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={editForm.client_paralegal_processor}
                onChange={(e) => setEditForm({ ...editForm, client_paralegal_processor: e.target.value })}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Marketing Points</label>
              <input
                type="number"
                value={editForm.marketing_points || ''}
                onChange={(e) => setEditForm({ ...editForm, marketing_points: e.target.value === '' ? 0 : parseInt(e.target.value) })}
                min="0"
                placeholder="0"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
