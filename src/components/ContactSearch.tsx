...
export function ContactSearch() {
  const { salesPerson, isAdminOrProcessor, user, salesPersonId } = useAuth();
    ...
    try {
      ...
      if (allContacts) {
        Object.entries(filters).forEach(([type, searchTerm]) => {
          ...
          const searchVariants = expandSearchTermWithNicknames(searchTerm.trim());

          contactsOfType.forEach(contact => {
            let maxSimilarity = 0;

            for (const variant of searchVariants) {
              const similarity = calculateSimilarity(contact.name, variant);
            ...
            if (maxSimilarity >= SIMILARITY_THRESHOLD) {
              ...
              if (!existingMatch || maxSimilarity > existingMatch.similarity) {
  ...
  const handleSaveEdit = async (contactId: string) => {
    try {
      const currentResult = results.find(r => r.id === contactId);
      if (!currentResult) {
      ...
      setToast({ message: `Failed to update contact: ${error.message}`, type: 'error' });
    }
  };

  const canEdit = (result: SearchResult) => {
    return result.salesPersonId === salesPerson?.id || hasAccess('edit_admin_fields');
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          <label htmlFor="loan_officer" className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
          ...
          <input
            ...
            onChange={(e) => handleFilterChange('loan_officer', e.target.value)}
            onKeyPress={handleKeyPress}
              ...
              >
                {isEditing ? (
                  <div className="space-y-4">
                    ...
                    {(isAdminOrProcessor || hasAccess('edit_admin_fields')) && (
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-slate-600 space-y-1">
                          ...
                          {(isAdminOrProcessor || hasAccess('edit_admin_fields')) && result.preferred_uw && (
                            <p className="text-amber-700 font-medium">Preferred UW: {result.preferred_uw}</p>
                          )}
                          {(isAdminOrProcessor || hasAccess('edit_admin_fields')) && result.preferred_closer && (
                          ...
                          {(isAdminOrProcessor || hasAccess('edit_admin_fields')) && (
                            <p className="text-amber-700 font-medium">
                              Processor Notes: {result.processor_notes || <span className="text-slate-400 italic">None</span>}
                            </p>
                          )}
                        </div>
                      </div>
